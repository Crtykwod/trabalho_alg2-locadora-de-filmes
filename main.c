...
#define NOME_ARQUIVO_TXT "banco_dados.txt"
...
int CancelarAssinatura(ASSINATURAS *BancoAssinaturas, int *total_assinaturas) {
  LimparTerminal();
  ...
  for (int i = 0; i < *total_assinaturas; i++) {
    if (strcasecmp(cpf, BancoAssinaturas[i].cpf_usuario_assinante) == 0 &&
      ...
      if (!BancoAssinaturas[i].ativa) {
        LimparTerminal();
        printf("-- Cancelar assinatura --\n\n");
        printf("Essa assinatura ja esta cancelada!\n");
        printf("Pressione ENTER para retornar.\n\n");
        getchar();
        LimparTerminal();
        return 0;
...
int MenuPrincipal(int opcao) {
  ...
  printf("4 - Gerar Arquivo de Relatorio.\n");
  printf("5 - Sair.\n\n");
  printf("Escolha uma opcao: ");
...
int GerenciamentoDeAssinaturas() {
  ...
  printf("4 - Cancelar uma assinatura.\n");
  printf("5 - Voltar.\n\n");
  printf("Escolha uma opcao: ");
...
int procurar_arquivo(USUARIOS **BancoUsuarios, int *total_clientes, PLATAFORMAS **BancoPlataformas, int *total_plataformas, ASSINATURAS **BancoAssinaturas, int *total_assinaturas) {
  ...
  FILE *fArquivoBin = fopen(NOME_ARQUIVO, "rb");
  int opcao_arquivo = 0;
...
int salvar_arquivo_bin(USUARIOS *BancoUsuarios, int total_clientes, PLATAFORMAS *BancoPlataformas, int total_plataformas, ASSINATURAS *BancoAssinaturas, int total_assinaturas) {

  FILE *fArquivoBin = fopen(NOME_ARQUIVO, "wb");

  if (fArquivoBin == NULL) {
    printf("Erro ao criar/abrir o arquivo para salvar os dados!\n");
    return 0; //return 0 se nao conseguiu abrir o arquivo
  }

  //salvando os clientes no arquivo
  fwrite(&total_clientes, sizeof(int), 1, fArquivoBin);

  if (total_clientes > 0) {
    fwrite(BancoUsuarios, sizeof(USUARIOS), total_clientes, fArquivoBin);
  }

  //salvando as plataformas no arquivo
  fwrite(&total_plataformas, sizeof(int), 1, fArquivoBin);

  if (total_plataformas > 0) {
    fwrite(BancoPlataformas, sizeof(PLATAFORMAS), total_plataformas, fArquivoBin);
  }

  //salvando as assinaturas no arquivo
  fwrite(&total_assinaturas, sizeof(int), 1, fArquivoBin);

  if (total_assinaturas > 0) {
    fwrite(BancoAssinaturas, sizeof(ASSINATURAS), total_assinaturas, fArquivoBin);
  }

  fclose(fArquivoBin);
  return 1; //retornando 1 pq o programa conseguiu abrir e salvar o arquivo com sucesso
}

int salvar_arquivo_txt(USUARIOS *BancoUsuarios, int total_clientes, PLATAFORMAS *BancoPlataformas, int total_plataformas, ASSINATURAS *BancoAssinaturas, int total_assinaturas) {

  FILE *fArquivoTxt = fopen(NOME_ARQUIVO_TXT, "w");

  if (fArquivoTxt == NULL) {
    printf("Erro ao criar/abrir o arquivo para salvar os dados!\n");
    exit(1); //return 0 se nao conseguiu abrir o arquivo
  }

  fprintf(fArquivoTxt, "== Clientes ==\n\n");
  for (int i = 0; i < total_clientes; i++) {
    fprintf(fArquivoTxt, "Nome: %s\n", BancoUsuarios[i].nome);
    fprintf(fArquivoTxt, "CPF: %s\n", BancoUsuarios[i].cpf);
    fprintf(fArquivoTxt, "ID do usuario: %ld\n", BancoUsuarios[i].id_usuario);
    fprintf(fArquivoTxt, "Telefone: %s\n", BancoUsuarios[i].phone);
    fprintf(fArquivoTxt, "Email: %s\n\n", BancoUsuarios[i].email);
  }

  fprintf(fArquivoTxt, "== Plataformas ==\n\n");
  for (int i = 0; i < total_plataformas; i++) {
    fprintf(fArquivoTxt, "Nome: %s\n", BancoPlataformas[i].nome_plataforma);
    fprintf(fArquivoTxt, "ID da plataforma: %ld\n", BancoPlataformas[i].id_plataforma);
    fprintf(fArquivoTxt, "Categoria: %s\n", BancoPlataformas[i].categoria);
    fprintf(fArquivoTxt, "Preco: R$%.2f\n", BancoPlataformas[i].preco);
    fprintf(fArquivoTxt, "Site URL: %s\n\n", BancoPlataformas[i].site_url);
  }

  fprintf(fArquivoTxt, "== Assinaturas ==\n\n");
  for (int i = 0; i < total_assinaturas; i++) {
    fprintf(fArquivoTxt, "CPF do usuario: %s\n", BancoAssinaturas[i].cpf_usuario_assinante);
    fprintf(fArquivoTxt, "Plataforma contratada: %s\n", BancoAssinaturas[i].nome_plataforma_contratada);
  }
...
int main() {
  ...
  while (1) {
    ...
    case 3:
      ...
      switch (opcao) {
      case 1:
        ASSINATURAS * temp = realloc(pBancoAssinaturas, (total_assinaturas + 1) * sizeof(ASSINATURAS));

        if (temp == NULL) {
          printf("Erro ao alocar memoria para as assinaturas!\n");
          exit(1);
        }

        pBancoAssinaturas = temp;
      ...
      case 3:
        ...
        break;
      case 4:
        CancelarAssinatura(pBancoAssinaturas, &total_assinaturas);
      default:
        continue;
      }
      break;
    ...
    case 4:
      salvar_arquivo_bin(pBancoUsuarios, total_clientes, pBancoPlataformas, total_plataformas, pBancoAssinaturas, total_assinaturas);
      salvar_arquivo_txt(pBancoUsuarios, total_clientes, pBancoPlataformas, total_plataformas, pBancoAssinaturas, total_assinaturas);
    ...
    default:
      salvar_arquivo_bin(pBancoUsuarios, total_clientes, pBancoPlataformas, total_plataformas, pBancoAssinaturas, total_assinaturas);
      free(pBancoUsuarios);
      free(pBancoPlataformas);
      free(pBancoAssinaturas);
      exit(0);
    }
  }
